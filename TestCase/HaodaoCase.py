...
class Test_haidao_reg_case:
    ...
    def test_商品增加测试001(self):
        with allure.step('步骤1:打开后台地址'):
            self.浏览器.打开地址('http://39.102.208.214/haidao/index.php?m=admin&c=index&a=index')
        ...
        with allure.step('步骤3:完成增加商品信息内容'):
            ...
            self.浏览器.清除内容('name','shop_price[]')
            self.浏览器.输入内容('name','shop_price[]','100'+str(random.randint(1000,9999)))
            self.浏览器.清除内容('name','market_price[]')
            self.浏览器.输入内容('name','market_price[]','20'+str(random.randint(1000,9999)))
            self.浏览器.输入内容('name','number[]','50'+str(random.randint(1000,9999)))
            time.sleep(1)
            ...
            self.浏览器.输入内容('xpath','/html/body/div[2]/div/form/div[1]/div[2]/div','6'+str(random.randint(1000,9999)))#商品货号
    ...
    def test_商品购买测试001(self):
        ...
        with allure.step('步骤5:购买商品'):
            ...
            #商品购买
            self.浏览器.点击元素('xpath','/html/body/div[5]/div[1]/div/ul/li[1]/div[1]/a') #点击图片
            self.浏览器.点击元素('text','立即购买')
            self.浏览器.点击元素('xpath','/html/body/div[3]/div[2]/dl[1]/dd/div/a')
            self.浏览器.切换框架('xpath','/html/body/div[7]/div/table/tbody/tr[2]/td/div/iframe')
            self.浏览器.点击元素('xpath','/html/body/form/div[1]/ul/li[1]/div/div/div[2]/div[1]')
            self.浏览器.点击元素('text','广西')
            self.浏览器.点击元素('text','南宁')
            self.浏览器.点击元素('text','西乡塘')
            self.浏览器.点击元素('text','西乡塘街道')
            self.浏览器.输入内容('name','address','榕华大厦313')
            self.浏览器.输入内容('name','name','李四')
            self.浏览器.输入内容('name','mobile','13888888888')
            self.浏览器.输入内容('name','zipcode','533899')
